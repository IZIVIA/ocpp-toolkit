...
class AdapterTest {
    ...
    fun `diagnostics status notification request`() {
        ...
        // The two terminal states OCPP 1.2 models are forwarded as such.
        mapOf(
    ...
    fun `transient diagnostics status is ignored and not forwarded in OCPP 1_2`() {
        ...
        // OCPP 1.2 has no transient diagnostics state, so neither is forwarded.
    ...
    fun `stopping a transaction releases its id mapping`() {
        ...
        val adapter = Ocpp12Adapter("CP001", transport, csApi, transactionIds)
